...
main (int argc, char **argv)
{
  ...
  int SDC_flag=0;
  int cont=0;
 // char* argv[]={"cjpeg", "-dct", "int", "-progressive", "-opt", "-outfile", "/benchmarks/output_large_encode.jpeg", "/benchmarks/input_large.ppm"};
  //int argc = 8;
  ...
  while(1){
    ...
    for(ex=0;ex<NUM_EXEC;ex++){
       ...
       // printf("11");
        ...
        if ((golden=fopen("/benchmarks/golden.jpeg","rb")) == NULL) {
          fprintf(stderr, "%s: error check can't open %s\n", progname, "golden.jpeg");
          exit(EXIT_FAILURE);
        }
        if ((output_file = fopen(outfilename, "rb")) == NULL) {
          fprintf(stderr, "%s: error check can't open %s\n", progname, outfilename);
          exit(EXIT_FAILURE);
        }
        index=0;
        SDC_flag=0;
       // printf("16");
        while(fread(&golden_buf,1,1,golden)==1 && fread(&out_buf,1,1,output_file)==1){
