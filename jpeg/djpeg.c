 ...
 */
{
  ...
  for (argn = 3; argn < argc; argn++) {
	...
	if ((mapfile = fopen(argv[argn], "rb")) == NULL) {
...
main (int argc, char **argv)
{
 ...
 //  char* argv[]={"djpeg", "-dct", "int", "-ppm", "-outfile", "/benchmarks/output_large_decode.ppm", "/benchmarks/input_large.jpg"};
  //int argc = 7;
...
while(1){
      ...
      fclose(input_file);
       fclose(output_file);
        ...
        FILE *golden;
        if ((golden=fopen("/benchmarks/golden.ppm","rb")) == NULL) {
          fprintf(stderr, "%s: can't open %s\n", progname, "golden.jpeg");
          exit(EXIT_FAILURE);
        }
        output_file=fopen(outfilename,"rb");
        int index=0;
        char golden_buf;
        char out_buf;
        int SDC_flag=0;
